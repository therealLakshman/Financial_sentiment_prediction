...
def main():
    ...
    # ✅ Explicitly use the experiment ID when starting run
    with mlflow.start_run(experiment_id=exp_id) as run:
        print(f"Started run {run.info.run_id} under experiment {run.info.experiment_id}")           
        try:
            ...
            model = load_model(os.path.join(root_dir, 'decisiontree_model.pkl'))
            vectorizer = load_vectorizer(os.path.join(root_dir, 'bow_vectorizer.pkl'))

            # Load test data for signature inference
            test_data = load_data(os.path.join(root_dir, 'src/datasets/interim/test_processed.csv'))

            # Prepare test data
            X_test_bow = vectorizer.transform(test_data['Sentence'].values)
            ...
            model_path = "DecisionTree_model"
            run_id_ = run.info.run_id
            print("run_id that actually sending to experiment",run_id_)
            save_model_info(run_id_, model_path, 'experiment_info.json')
