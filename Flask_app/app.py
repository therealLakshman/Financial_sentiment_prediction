...
model, vectorizer = load_model("./decisiontree_model.pkl", "./bow_vectorizer.pkl")  
...
def predict_with_timestamps():
    ...
    try:
        ...
        # Original predictions might be like ['positive', 'negative', 'neutral']
        raw_predictions = model.predict(dense_comments)

        print("Raw predictions from model:", raw_predictions)
        if raw_predictions.any():
            print("Type of a single prediction:", type(raw_predictions[0]))
        
        # --- START: ADD THIS FIX ---
        # Define a mapping from string labels to the numbers your frontend expects
        # This is the correct mapping based on your model's output
        label_mapping = {
                'positive': '1',
                'neutral': '0',
                'negative': '-1'
        ...
        # Apply the mapping. Default to '0' if an unexpected label appears.
        predictions = [label_mapping.get(pred, '0') for pred in raw_predictions]
        # --- END: FIX ---
...
def generate_wordcloud():
    try:
        ...
        if not comments:
            return jsonify({"error": "No comments provided for word cloud"}), 400

        # Join all comments into a single string
        text = ' '.join(comments)
        
        if not text.strip():
             return jsonify({"error": "Comment text is empty, cannot generate word cloud"}), 400
