...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const API_KEY = 'AIzaSyDze6fv4jCXMr_tqfhUj7a4UIWJnDPGTZo';  // Replace with your actual YouTube Data API key
  // const API_URL = 'http://my-elb-2062136355.us-east-1.elb.amazonaws.com:80';   
  const API_URL = 'financial-sentiment-lb-1013353912.ap-south-1.elb.amazonaws.com';
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (match && match[1]) {
      ...
      if (predictions) {
        ...
        //     </ul>
        //   </div>`;
        // Add the top comments section with text labels
        ...
        <div class="section">
            ...
            ${predictions.slice(0, 25).map((item, index) => `
                <li class="comment-item">
                <span>${index + 1}. ${item.comment}</span><br>
                <span class="comment-sentiment">Sentiment: ${sentimentLabels[item.sentiment]}</span>
                </li>`).join('')}
            </ul>
        </div>`;
      }
